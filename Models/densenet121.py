...
import tensorflow as tf
import keras
from custom_callback import Save_Accuracy_By_Epoch
...
def kernel_init():
    return tf.keras.initializers.RandomNormal(mean=0.0, stddev=0.05, seed=None)

OldModel = tf.keras.applications.DenseNet121(include_top=False,
input_tensor=None,
weights='imagenet')
...
model.add(tf.keras.layers.Conv2D(3,(5,5),activation = "relu" , input_shape = (128,128,3)))
model.add(OldModel)
...
model.fit(train_images_raw, train_labels, epochs=100, callbacks=[Save_Accuracy_By_Epoch((test_images_raw,test_labels))])
