...
from preprocessing.image_modules import *
import tensorflow as tf
import keras
...
for i in [100]:
    ...
    history = model.fit(train_images, train_labels, epochs=i,callbacks=[Save_Multiclass_Metrics_By_Epoch((test_images_raw,test_labels),6)])
    ...
    y_pred_ann = model.predict(test_images)
    y_pred = get_labels(y_pred_ann)

    accuracy = find_accuracy(test_labels, y_pred)    
